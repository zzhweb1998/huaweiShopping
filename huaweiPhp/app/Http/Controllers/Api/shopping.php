...
class shopping extends Controller
{
    ...
    public function Login(Request $request){
        ...
        $loginIf = $request->get('loginIf'); //0为用户验证，1为管理员验证
        $msg = '';
        if($loginIf == 0){
            $data = \App\Home\user::where('user_Grade','<','6')->get();
    ...
    public function Register(Request $request){
        ...
        $registerIf = $request->get('registerIf');//0为用户验证，1为管理员验证
        $msg = '';
        if($registerIf == 0){
            $data = \App\Home\user::where('user_Grade','<','6')->get();
            foreach($data as $key => $value){
    ...
    public function newOrders(Request $request){
        ...
        $data = \App\Home\user::where('user_account','=',$account)->where('user_Grade','<','6')->select('user_id')->get();
        $userId = $data[0]->user_id;
        $data = DB::insert("insert into orders(user_id) values(?)",[$userId]);
        echo $data;
    }
    //3、商品页面把商品直接提交到预备订单
    public function submitGoods(Request $request){
        ...
        $data = \App\Home\goods::where('goods_no','=',$no)->select('goods_id')->get();
        $goodsId = $data[0]->goods_id;
        $data = DB::insert("insert into shopcar(shopcar_status,goods_id,user_id) values(?,?,?)",[2,$goodsId,$userId]);
        echo $data;
    }

    //获取指定用户所有订单详情信息
    public function allOD(Request $request){
        $account = $request->get('account');
        $data = \App\Home\ordersDetailed::where('user_account','=',$account)->get();
        echo $data;
    }

    //获取指定用户所有订单商品信息
    public function allOG(Request $request){
        $account = $request->get('account');
        $data = \App\Home\ordersGoods::where('user_account','=',$account)->get();
        echo $data;
    }
    ...
    public function applyRefund(Request $request){
        $id = $request->get('id');
        $con = $request->get('con');
        $data = DB::insert("insert into refund(shopcar_id,refund_content) values(?,?)",[$id,$con]);
        echo $data;
    }
    ...
    public function queryRefund(Request $request){
        $id = $request->get('id');
        $data = \App\Home\refund::where('shopcar_id','=',$id)->where('refund_status','=','1')->get();
        echo $data;
    }
    ...
    public function statistics(){
        $data = \App\Home\adminStatistics::get();
        echo $data;
    }
}
