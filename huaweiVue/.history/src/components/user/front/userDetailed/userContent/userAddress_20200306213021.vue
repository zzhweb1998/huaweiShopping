<template>
    <div id="userAddress">
        <div class="addressTitle">送货地址管理</div>
        <div class="addressOperate">
            <div>
                <span>收件人：</span>
                <input type="text" v-model="addressName">
            </div>
            <div>
                <span>手机号码：</span>
                <input type="text" v-model="addressPhone">
            </div>
            <div>
                <span>送货地址：</span>
                <div class="ao-content">
                    <v-distpicker :province="placeholders.province" :city="placeholders.city" :area="placeholders.area"
                        @selected="onSelected"></v-distpicker>
                    <textarea cols="50" rows="3" placeholder="输入详细地址信息" v-model="addressDetailed"></textarea>
                </div>
            </div>
            <div>
                <span>邮编：</span>
                <input type="text" v-model="addressZip">
            </div>
            <div v-show="operateIf == 0">
                <button @click="addAddress">添加新地址</button>
                <button @click="clearText">清 空</button>
            </div>
            <div v-show="operateIf == 1">
                <button @click="updAddress">确 定</button>
                <button @click="cutOperate">取 消</button>
            </div>
        </div>
        <div class="addressBox">
            <div class="uc-table">
                <table border="1" cellspacing="0" cellpadding="0">
                    <tr>
                        <th>收件人</th>
                        <th>手机号码</th>
                        <th>地址详情</th>
                        <th>邮编</th>
                        <th>操作</th>
                    </tr>
                    <tr v-for="(ai,index) in addressInfo" :key="index">
                        <td>{{ai.user_name}}</td>
                        <td>{{ai.user_phone}}</td>
                        <td>{{ai.address_province}} {{ai.address_city}} {{ai.address_district}} {{ai.address_detailed}}
                        </td>
                        <td>{{ai.address_zip}}</td>
                        <td>
                            <span class="uc-operate" @click="editAddress(ai)">编辑</span>
                            <span class="uc-operate" @click="delAddress(ai.address_id)">删除</span>
                            <span class="uc-operate" v-show="ai.address_id != defaultId"
                                @click="defAddress(ai.address_id)">设为默认地址</span>
                        </td>
                    </tr>
                </table>
            </div>
        </div>
    </div>
</template>

<script>
    import VDistpicker from 'v-distpicker'

    export default {
        data() {
            return {
                userAccount: '',
                addressInfo: '',
                defaultId: '',
                placeholders: {
                    province: '',
                    city: '',
                    area: '',
                },
                addressName: '',
                addressPhone: '',
                addressDetailed: '',
                addressZip: '',
                operateIf: 0,
                addressId: ''
            }
        },
        created() {
            this.userAccount = sessionStorage.getItem('account')
            this.axios.post("/api/shop/receiveAddress", {
                account: this.userAccount
            }).then(res => {
                this.addressInfo = res.data
            })
            this.axios.post("/api/shop/userInfo", {
                account: this.userAccount
            }).then(res => {
                this.defaultId = res.data[0].user_address_id
            })
        },
        methods: {
            pageRefresh() {
                this.axios.post("/api/shop/receiveAddress", {
                    account: this.userAccount
                }).then(res => {
                    this.addressInfo = res.data
                })
            },
            onSelected(data) {
                this.placeholders.province = data.province.value
                this.placeholders.city = data.city.value
                this.placeholders.area = data.area.value
            },
            clearText() {
                this.placeholders.province = ''
                this.placeholders.city = ''
                this.placeholders.area = ''
                this.addressName = ''
                this.addressPhone = ''
                this.addressDetailed = ''
                this.addressZip = ''
            },
            cutOperate() {
                this.clearText()
                this.operateIf = 0
            },
            editAddress(ai) {
                this.placeholders.province = ai.address_province
                this.placeholders.city = ai.address_city
                this.placeholders.area = ai.address_district
                this.addressName = ai.user_name
                this.addressPhone = ai.user_phone
                this.addressDetailed = ai.address_detailed
                this.addressZip = ai.address_zip
                this.operateIf = 1
                this.addressId = ai.address_id
            },
            addAddress() {
                var phoneReg = /^1([38]\d|5[0-35-9]|7[3678])\d{8}$/
                var zipReg= /^[1-9][0-9]{5}$/
                if (this.addressName == '' || this.addressName == null) {
                    alert('请输入收件人')
                } else if (this.addressPhone == '' || this.addressPhone == null) {
                    alert('请输入手机号码')
                } else if (!phoneReg.test(this.addressPhone)) {
                    alert('请输入正确的手机号')
                } else if (this.placeholders.province == '' || this.placeholders.province == null
                    || this.placeholders.city == '' || this.placeholders.city == null
                    || this.placeholders.area == '' || this.placeholders.area == null) {
                    alert('请选择地区')
                } else if (this.addressDetailed == '' || this.addressDetailed == null) {
                    alert('请输入地址详情')
                } else if (this.addressZip == '' || this.addressZip == null) {
                    alert('请输入邮编')
                } else if (!zipReg.test(this.addressZip)) {
                    alert('请输入正确的邮编')
                } else {
                    this.axios.post("/api/shop/addAddress", {
                        name: this.addressName,
                        phone: this.addressPhone,
                        province: this.placeholders.province,
                        city: this.placeholders.city,
                        area: this.placeholders.area,
                        detailed: this.addressDetailed,
                        zip: this.addressZip,
                        account: this.userAccount
                    }).then(res => {
                        if (res.data == 1) {
                            alert('地址添加成功')
                            this.axios.post("/api/shop/userInfo", {
                                account: this.userAccount
                            }).then(res => {
                                this.defaultId = res.data[0].user_address_id
                            })
                            this.pageRefresh()
                            this.clearText()
                        } else {
                            alert('地址添加失败')
                        }
                    })
                }
            },
            updAddress() {
                var phoneReg = /^1([38]\d|5[0-35-9]|7[3678])\d{8}$/
                var zipReg= /^[1-9][0-9]{5}$/
                if (this.addressName == '' || this.addressName == null) {
                    alert('请输入收件人')
                } else if (this.addressPhone == '' || this.addressPhone == null) {
                    alert('请输入手机号码')
                } else if (!phoneReg.test(this.addressPhone)) {
                    alert('请输入正确的手机号')
                } else if (this.placeholders.province == '' || this.placeholders.province == null
                    || this.placeholders.city == '' || this.placeholders.city == null
                    || this.placeholders.area == '' || this.placeholders.area == null) {
                    alert('请选择地区')
                } else if (this.addressDetailed == '' || this.addressDetailed == null) {
                    alert('请输入地址详情')
                } else if (this.addressZip == '' || this.addressZip == null) {
                    alert('请输入邮编')
                } else if (!zipReg.test(this.addressZip)) {
                    alert('请输入正确的邮编')
                } else {
                    this.axios.post("/api/shop/updateAddress", {
                        name: this.addressName,
                        phone: this.addressPhone,
                        province: this.placeholders.province,
                        city: this.placeholders.city,
                        area: this.placeholders.area,
                        detailed: this.addressDetailed,
                        zip: this.addressZip,
                        id: this.addressId
                    }).then(res => {
                        if (res.data == 1) {
                            alert('地址修改成功')
                            this.pageRefresh()
                            this.clearText()
                        } else {
                            alert('地址修改失败')
                        }
                    })
                }
            },
            delAddress(id) {
                this.axios.post("/api/shop/deteleAddress", {
                    id: id,
                    account: this.userAccount
                }).then(res => {
                    if (res.data == 1) {
                        alert('地址删除成功')
                        this.pageRefresh()
                        this.clearText()
                    } else {
                        alert('地址删除失败')
                    }
                })
            },
            defAddress(id) {
                this.axios.post("/api/shop/defaultAddress", {
                    id: id,
                    account: this.userAccount
                }).then(res => {
                    if (res.data == 1) {
                        this.defaultId = id
                    }
                })
            }
        },
        components: {
            VDistpicker
        }
    }
</script>

<style>
    #userAddress {
        width: 100%;
        height: 100%;
    }

    .addressTitle {
        width: 100%;
        height: 50px;
        line-height: 50px;
        font-size: 22px;
        text-indent: 30px;
        background: #eee;
    }

    .addressOperate {
        width: 100%;
        height: 300px;
        margin-top: 10px;
        border-bottom: 1px solid #eee;
    }

    .addressOperate>div {
        display: inline-block;
        width: 66%;
        height: auto;
        font-size: 20px;
        color: #555;
        margin-top: 10px;
    }

    .addressOperate>div:nth-of-type(n+5) {
        width: 300px;
        height: 40px;
    }

    .addressOperate>div:nth-of-type(n+5)>button {
        width: 120px;
        height: 40px;
        border: 1px solid #ccc;
        outline: none;
        cursor: pointer;
        font-size: 18px;
    }

    .addressOperate>div:nth-of-type(n+5)>button:nth-of-type(1) {
        background: #f99c11;
    }

    .addressOperate>div:nth-of-type(n+5)>button:nth-of-type(1):hover {
        background: #e88b00;
    }

    .addressOperate>div:nth-of-type(n+5)>button:nth-of-type(2) {
        background: #f5f5f5;
    }

    .addressOperate>div:nth-of-type(n+5)>button:nth-of-type(2):hover {
        background: #eee;
    }

    .addressOperate>div>span {
        display: inline-block;
        width: 150px;
        text-align: right;
    }

    .addressOperate>div>input[type=text] {
        width: 200px;
        height: 40px;
        font-size: 18px;
        text-indent: 10px;
    }

    .ao-content {
        display: inline-block;
        width: 520px;
        height: 120px;
    }


    .addressOperate>div:nth-of-type(3)>span {
        vertical-align: top;
    }

    .ao-content>textarea {
        font-size: 18px;
        margin-top: 5px;
        outline: none;
        text-indent: 10px;
    }

    .addressBox {
        width: 100%;
        height: 180px;
        margin-top: 5px;
        overflow: hidden;
        overflow-y: scroll;
    }

    .addressHeader {
        width: 100%;
        height: 40px;
        background: #f5f5f5;
    }

    .addressInfo {
        width: 100%;
        height: 140px;
        border: 1px solid #333;
        overflow: hidden;
        overflow-y: scroll;
    }

    .uc-table {
        width: 100%;
        height: auto;
        margin-top: 10px;
    }

    .uc-table>table {
        width: 98%;
        margin: 0 auto;
    }

    .uc-table tr {
        height: 30px;
        text-align: center;
    }

    .uc-operate {
        font-size: 14px;
        cursor: pointer;
    }

    .uc-operate:nth-of-type(n+2)::before {
        content: '|';
        font-size: 14px;
        margin-right: 5px;
    }

    .uc-operate:hover {
        color: crimson;
    }
</style>