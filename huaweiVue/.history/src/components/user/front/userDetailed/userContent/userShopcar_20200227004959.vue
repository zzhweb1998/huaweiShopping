<template>
    <div id="userShopcar">
        ...
        <div class="us-content" v-for="(gi,index) in goodsInfo" :key="index">
            <div class="goods-box">
                <div class="gb-sel"></div>
                <div class="gb-img"></div>
                <div class="gb-info"></div>
                <div class="gb-price"></div>
                <div class="gb-num">
                    <input type="number" min="1" step="1" @change="numIf(index)" v-model="goodsNum">
                    <div class="num-cut" @click="numReduce(index)">-</div>
                    <div class="num-add" @click="numAdd">+</div>
                </div>
                <div class="gb-total"></div>
                <div class="gb-operate"></div>
            </div>
        </div>
    </div>
</template>
        ...
        methods:{
            ...
            numIf(index) {
                ...
                if (!regNum.test(this.goodsNum)) {
                    this.goodsNum = 1
                }else if(this.goodsNum > this.goodsInfo[index - 1].stock_num){
                    this.goodsNum = 1
                }
            },
            numAdd(index){
                if(this.goodsNum < this.goodsInfo[index - 1].stock_num){
                    this.goodsNum ++ 
                }
            },
            numReduce(){
                console.log(this.goodsInfo[index - 1].stock_num);
