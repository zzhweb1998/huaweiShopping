<template>
    <div id="userOrders">
            ...
            <div class="uo-info">
                <div class="uo-box" v-for="(oi,index) in ordersShow" :key="index">
                    ...
                    <div class="orders-operate">
                        <div v-if="oi.orders_status == 0" @click="payModel(oi.orders_no)">立即付款</div>
                    ...
                    <div class="ordersPay" v-show="payIf == oi.orders_no">
                        ...
                        <div>
                            <button @click="immediatelyPay(oi.orders_no,oi.total_price)">确认支付</button>
        ...
        <div class="pageBan" v-show="banIf == 1"></div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            ...
            payModel(no) {
                if (this.userInfo.user_pay == '' || this.userInfo.user_pay == null) {
                    alert('请设置付款密码')
                } else {
                    this.banIf = 1
                    this.payIf = no
            ...
            immediatelyPay(no, total) {
                if (this.payPassword == '' || this.payPassword == null) {
                    alert('请输入付款密码')
                } else if (this.payPassword != this.userInfo.user_password) {
                    alert('付款密码有误请重新输入')
                    this.payPassword = ''
                } else if (this.userInfo.user_money < price) {
                    alert('用户余额不足，请充值余额')
                } else {
                    alert('付款成功')
                }
