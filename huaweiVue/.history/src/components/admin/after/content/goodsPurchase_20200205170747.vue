    ...
    <div id="goodsPurchase">
        ...
        <div class="purchase-content">
            <div class="purchase-info">
                ...
                <p>商品名称：{{goodsInfo.goods_info_name}}</p>
                <p>商品颜色：{{goodsInfo.goods_color}}</p>
                <p>商品配置：{{goodsInfo.goods_configure}}</p>
                <p>商品库存：{{goodsInfo.stock_num}}</p>
                <p>进货总数：{{goodsInfo.purchase_sum}}</p>
                <p>进货总额：{{goodsInfo.purchase_total}}</p>
            </div>
            <div class="purchase-operate">
                ...
                <button @click="goodsPurchase()">进货</button>
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            priceIf() {
                ...
                if (!priceReg.test(this.purchasePrice)) {
                    this.purchasePrice = 0
                }
            },
            goodsPurchase() {
                if (this.purchaseNum.trim() == '' || this.purchaseNum.trim() == null) {
                    alert('请输入进货数量')
                } else if (this.purchasePrice.trim() == '' || this.purchasePrice.trim() == null) {
                    lert('请输入进货价格')
                } else {
                    this.axios.post("/api/shop/goodsPurchase", {
                        id: this.goodsInfo[0].stock_id,
                        num: this.purchaseNum,
                        price: this.purchasePrice
                    }).then(res => {
                        if (res.data == 1) {
                            alert('商品进货成功')
                            this.$router.push('/admin/goodsStock')
                        } else {
                            alert('商品进货失败')
                        }
                    })
