<template>
    <div id="purchaseInfo">
        ...
        <div class="ac-search">
            ...
            <div class="operate-box" v-show="revokeShow>0">
                <span>进货单号:{{purchaseId}}</span>
                <span>撤销理由：</span>
                <textarea cols="20" rows="2" v-model="revokeReason"></textarea>
                <button v-show="revokeShow==1" @click="revoke">撤销</button>
            </div>
        </div>
        <div class="ac-table">
            <table border="1" cellspacing="0" cellpadding="0">
                ...
                <tr v-for="(ps,index) in pageShow[pageCurrent]" :key="index">
                    ...
                    <td>
                        <button class="ac-operate" v-if="ps.purchase_status == 1"
                            @click="revokeOperate(ps.purchase_id)">撤销</button>
                        <button class="ac-operate" v-if="ps.purchase_status == 0"
                            @click="revokeSee(ps.purchase_id)">查看</button>
                    </td>
                </tr>
            </table>
            ...
            revokeSee(id) {
                ...
                this.conInfo.filter(item => {
                    if (item.purchase_id == id) {
                        this.revokeReason = item.revokeReason
                    }
                })
            },
            revoke() {
                if (this.revokeReason.trim() == '' || this.revokeReason.trim() == null) {
                    alert('请输入撤销理由')
                } else {
                    let revokeIf = confirm('是否撤销此订单')
                    if (revokeIf == true) {
                        this.axios.post("/api/shop/revokePurchase", {
                            id: this.purchaseId,
...
<style>
    ...
    .operate-box>span:nth-of-type(2) {
        margin-left: 20px;
    }

    .operate-box>textarea {
